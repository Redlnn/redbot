...
from .whitelist.delete import del_whitelist_by_id, del_whitelist_by_qq, del_whitelist_by_uuid
...
@channel.use(
    ListenerSchema(
        listening_events=[GroupMessage],
        inline_dispatchers=[Twilight(Sparkle([RegexMatch(r'[!！.]wl\ del\ '), WildcardMatch()]))],
        decorators=[
            GroupPermission.require(
                MemberPerm.Administrator,
                send_alert=True,
...
@channel.use(
    ListenerSchema(
        listening_events=[GroupMessage],
        inline_dispatchers=[Twilight(Sparkle([RegexMatch(r'[!！.]wl\ info\ '), WildcardMatch()]))],
        decorators=[GroupPermission.require()],
    )
)
...
@channel.use(
    ListenerSchema(
        listening_events=[GroupMessage],
        inline_dispatchers=[Twilight(Sparkle([RegexMatch(r'[!！.]myid\ '), WildcardMatch()]))],
        decorators=[GroupPermission.require()],
    )
)
...
async def pardon(app: Ariadne, group: Group, message: MessageChain, source: Source):
    ...
    if player.uuid1 is not None:
        try:
            ...
            app.sendMessage(group, MessageChain.create(Plain(f'无法查询【{player.uuid1}】对应的正版id: 👇\n{e}')), quote=source)
    ...
    if player.uuid2:
        try:
            ...
            app.sendMessage(group, MessageChain.create(Plain(f'无法查询【{player.uuid2}】对应的正版id: 👇\n{e}')), quote=source)
...
async def ban(app: Ariadne, group: Group, message: MessageChain, source: Source):
    ...
    if player.uuid2:
        try:
            ...
            app.sendMessage(group, MessageChain.create(Plain(f'无法查询【{player.uuid21}】对应的正版id: 👇\n{e}')), quote=source)
            logger.error(f'无法查询【{player.uuid2}】对应的正版id')
            logger.exception(e)
            flags.append(False)
        else:
            if isinstance(mc_id, str):
                ...
                flags.append(False)
    await app.sendMessage(group, await del_whitelist_by_qq(int(target), group), quote=source)
    if False not in flags:
        await app.sendMessage(group, MessageChain.create(Plain('已封禁该玩家')), quote=source)
    else:
