...
async def add_whitelist_to_qq(qq: int, mc_id: str, admin: bool) -> MessageChain:
    ...
    if player is None:
        ...
        return MessageChain(Plain('这个id本来就是你哒'))
    else:
    ...
    if player.uuid1 is None and player.uuid2 is None:
        ...
        if admin:
            ...
            return MessageChain(Plain('你已有一个白名单，如要申请第二个白名单请联系管理员'))
        ...
        return MessageChain(Plain('目标玩家已有两个白名单，如需添加白名单请删除至少一个'))
    else:
        return MessageChain(Plain('你已经有两个白名单了噢'))

    try:
        res: str = await execute_command(f'whitelist add {real_mc_id}')
    except TimeoutError:
        return MessageChain(Plain('添加白名单时已写入数据库，但连接服务器超时，请联系管理解决'))
    except ValueError as e:
        logger.exception(e)
        return MessageChain(Plain(f'添加白名单时已写入数据库但无法连接到服务器，请联系管理解决: 👇\n{e}'))

    if res.startswith('Added'):
        return MessageChain(At(qq), Plain(' 呐呐呐，白名单给你!'))
    else:
