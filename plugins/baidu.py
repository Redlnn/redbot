...
async def friend_message_listener(
    ...
    group: Group, message: MessageChain
):
    m = message.asDisplay()
    if m.endswith("帮楼上百度"):
        if message.has(Quote):
            ...
            if quote.senderId != QQ:
                ...
                await app.sendGroupMessage(
                    ...
                    MessageChain.create([
                        Plain("请在这里找答案哦:\n"),
                        Plain("https://www.baidu.com/s?wd="),
                        Plain(parse.quote(ctx))
                    ]),
                    quote=source
                )
            else:
                await app.sendGroupMessage(
                    group, MessageChain.create([Plain("竹竹知道自己说的是什么东西啦~")])
                )
